<?php
Class ThemeHeaderStyle1 {

    public static $path = 'theme-header/header-style/header-style-1/';
    ...
    static function render() {
        ...
        <header class="">
            <!-- top bar -->
            <?php
            do_action('cle_header_top_bar');
            Template::partial(self::$path.'header-html', $headerData);
            do_action('cle_header_navigation');
            ?>
        </header>
        <?php
    }
    ...
    static function css() {
        ...
        $color = (!empty($background['color'])) ? $background['color'] : '#fff';
...
if(!Admin::is()) {
    add_action('cle_header_desktop', 'ThemeHeaderStyle1::render');
    add_action('theme_custom_css_no_tag', 'ThemeHeaderStyle1::css');
    add_action('theme_custom_script_no_tag', 'ThemeHeaderStyle1::script');
}
else {
    add_action('init', 'ThemeHeaderStyle1::navigation');
    add_action('theme_option_setup', 'ThemeHeaderStyle1::options', 20);
}