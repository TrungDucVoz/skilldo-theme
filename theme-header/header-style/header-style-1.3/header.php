<?php
Class ThemeHeaderStyle1_3 {

    public static $path = 'theme-header/header-style/header-style-1.3/';

    static function render() {
        ?>
        <header class="">
            <!-- top bar -->
            <?php do_action('cle_header_top_bar'); Template::partial(self::$path.'header-html'); do_action('cle_header_navigation');?>
        </header>
        <?php
    }
...
if(!Admin::is()) {
    add_action('cle_header_desktop', 'ThemeHeaderStyle1_3::render');
    add_action('theme_custom_css_no_tag', 'ThemeHeaderStyle1_3::css');
    add_action('theme_custom_script_no_tag', 'ThemeHeaderStyle1_3::script');
}
else {
    add_action('init', 'ThemeHeaderStyle1_3::navigation');
    add_action('theme_option_setup', 'ThemeHeaderStyle1_3::options', 20);
}