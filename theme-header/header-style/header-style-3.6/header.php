<?php
Class ThemeHeaderStyle3_6 {

    public static $path = 'theme-header/header-style/header-style-3.6/';
    ...
    static function render() {
        ...
        $headerData = [
            ...
            'text' => (!empty(Option::get('header_text'))) ? Option::get('header_text') : 'ĐĂNG KÝ NHẬN TIN',
            'textUrl' => (!empty(Option::get('header_text_url'))) ? Option::get('header_text_url') : '',
        ];
        ?>
    ...
    static function css() {
        ...
        $headerCssData = [
            ...
            'background'    => $css,
            'searchBg'      => (empty(Option::get('search_icon_bg'))) ? Option::get('theme_color') : Option::get('search_icon_bg'),
            'searchColor'   => (empty(Option::get('search_icon_color'))) ? '#fff' : Option::get('search_icon_color'),
            'hotlineBg'     => (empty(Option::get('hotline_icon_bg'))) ? Option::get('theme_color') : Option::get('hotline_icon_bg'),
            'hotlineColor'  => (empty(Option::get('hotline_icon_color'))) ? '#fff' : Option::get('hotline_icon_color'),
            'txtColor'      => (empty(Option::get('header_text_color'))) ? '#000' : Option::get('header_text_color'),
        ];
...
if(!Admin::is()) {
    add_action('cle_header_desktop', 'ThemeHeaderStyle3_6::render');
    add_action('theme_custom_css_no_tag', 'ThemeHeaderStyle3_6::css');
    add_action('theme_custom_script_no_tag', 'ThemeHeaderStyle3_6::script');
}
else {
    add_action('init', 'ThemeHeaderStyle3_6::navigation');
    add_action('theme_option_setup', 'ThemeHeaderStyle3_6::options', 30);
}