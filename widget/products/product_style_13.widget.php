<?php
class widget_product_style_13 extends widget {
    function __construct() {
        parent::__construct('widget_product_style_13', 'Sản phẩm (style 13)', ['container' => true, 'position'  => 'right']);
    ...
    function form( $left = [], $right = []) {
        ...
        $left[] = [
            'field' => 'subBg', 'label' => 'Màu nền danh mục', 'type'  => 'color',
            'after' => '<div class="col-md-6 form-group group">', 'before'=> '</div>'
        ];
        $left[] = [
            'field' => 'subColor', 'label' => 'Màu chữ danh mục', 'type'  => 'color',
            'after' => '<div class="col-md-6 form-group group">', 'before'=> '</div>'
        ];
        $left[] = [
            'field' => 'subBgHover', 'label' => 'Màu nền danh mục (hover)', 'type'  => 'color',
            'after' => '<div class="col-md-6 form-group group">', 'before'=> '</div>'
        ];
        $left[] = [
            'field' => 'subColorHover', 'label' => 'Màu chữ danh mục (hover)', 'type'  => 'color',
            'after' => '<div class="col-md-6 form-group group">', 'before'=> '</div>'
        ];

        $left[] = ['field' => 'display', 'type' => 'widget_product_style_13::inputDisplay'];
    ...
    function widget() {
        ...
        if(have_posts($this->options->pr_cate_sub) ) {
            ...
            if(have_posts($sub_categories)) $main_category = $sub_categories[0]->id;
        }
        $box = $this->container_box('widget_product_style_13');
        echo $box['before'];
        ?>
        <div class="box-header">
            <?php ThemeWidget::heading($this->name, (isset($this->options->heading)) ? $this->options->heading : [], '.js_'.$this->key.'_'.$this->id);?>
            <div class="product_style_13_header js_product_style_13_data" id="product_style_13_header_<?= $this->id;?>" data-run="false" data-id="<?= $this->id;?>" data-category="<?= $main_category;?>" data-options="<?php echo htmlentities(json_encode($this->options));?>">
                <div class="product_style_13_header_box <?php echo (($this->options->display['type'] == 1)?'noarrow':'') ?>">
                    <ul class="product_style_13_category_list">
                    ...
                    <ul class="product_style_13_arrow">
                        <li class="product_arrow"><a href="#" class="prev"><i class="fal fa-chevron-left"></i></a></li>
                        <li class="product_arrow"><a href="#" class="next"><i class="fal fa-chevron-right"></i></a></li>
                    </ul>
                    <?php } ?>
        ...
        <div class="product_style_13_content">
            <div class="box-content product-slider-horizontal" id="product_style_13_content_<?= $this->id;?>">
                <?php $this->loading();?>
                <?php
                    if($this->options->display['type'] == 0) { ?>
                        <div class="owl-carousel list-product"></div>
                        <style>
                            #product_style_13_content_<?= $this->id;?> .item {
                                margin-right: <?php echo $this->options->display['margin']/2;?>px;
                                margin-left: <?php echo $this->options->display['margin']/2;?>px;
                            }
                            #product_style_13_content_<?= $this->id;?> .slick-list {
                                margin-right: -<?php echo $this->options->display['margin']/2;?>px;
                                margin-left: -<?php echo $this->options->display['margin']/2;?>px;
                            }
                        </style>
        ...
        <div id="product_style_13_morelink_<?= $this->id;?>" class="text-center"><a href="<?= Url::permalink($slug);?>" class="more-link btn btn-theme btn-effect-default"><?php echo __('Xem tất cả');?></a></div>
        <div class="clearfix"></div>
        <style>
            .js_widget_product_style_13_<?php echo $this->id;?>.widget_product_style_13 .product_style_13_header_box .product_style_13_category_list li a {
                --pr13-sub-bg:<?php echo (!empty($this->options->subBg)) ? $this->options->subBg : '#b7b7b7';?>;
                --pr13-sub-txt:<?php echo (!empty($this->options->subColor)) ? $this->options->subColor : '#fff';?>;
                background-color: var(--pr13-sub-bg);
                color: var(--pr13-sub-txt);
            }
            .js_widget_product_style_13_<?php echo $this->id;?>.widget_product_style_13 .product_style_13_header_box .product_style_13_category_list li a:hover,
            .js_widget_product_style_13_<?php echo $this->id;?>.widget_product_style_13 .product_style_13_header_box .product_style_13_category_list li a.active {
                --pr13-sub-bg-hv:<?php echo (!empty($this->options->subBgHover)) ? $this->options->subBgHover : 'var(--theme-color)';?>;
                --pr13-sub-txt-hv:<?php echo (!empty($this->options->subColorHover)) ? $this->options->subColorHover : '#fff';?>;
                background-color: var(--pr13-sub-bg-hv);
                color: var(--pr13-sub-txt-hv);
            }
        </style>
        <?php
        echo $box['after'];
    ...
    function css() { include_once('assets/product-style-13.css'); }
    function script() { include_once('assets/product-script-13.js'); }
    function default() {
    ...
    static function loadProduct($ci, $model) {
        ...
        if(InputBuilder::post()) {
            ...
            if(have_posts($widgetData)) {
                ...
                $widget = new widget_product_style_13();
...
Widget::add('widget_product_style_13');
Ajax::client('widget_product_style_13::loadProduct');