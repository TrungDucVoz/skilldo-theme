$(function () {
    ...
    if('IntersectionObserver' in window) {
        let productListWidget = document.querySelectorAll('.js_product_style_5_data');
            ...
            entries.forEach(entry => {
                if(entry.isIntersecting) {
                    ...
                    if (typeof widgetLoad[id] == 'undefined') {
                        widgetLoad[id] = id;
                        product_style_5_slider_load(entry.target.getAttribute('data-category'), entry.target.getAttribute('data-id'), JSON.parse(entry.target.getAttribute('data-options')));
                    }
                }
            });
        });
        ...
        $('.widget_product_style_5 .js_product_style_5_data').each(function () {
            let options = $(this).data('options');
            product_style_5_slider_load($(this).data('category'), $(this).data('id'), options);
        });
    }

    $(document).on('click touch', '.product_style_5_category .product_style_5_category_list li.item a', function() {
        categoryId = $(this).attr('data-id');
        widgetId 	= $(this).attr('data-wg');
        $('#product_style_5_content_' + widgetId +' .product_style_5_category_list li.item a').removeClass('active');
        $(this).addClass('active');
        product_style_5_slider_load(categoryId, widgetId, $('#product_style_5_header_'+widgetId).data('options'));
        return false;
    });

    $('.product_style_5_header .js_btn_show_tab').click(function() {
        $(this).closest('.product_style_5_header_left').find('.product_style_5_category_list').toggle();
        return false;
    });

    function product_style_5_slider(id, options) {
        if(options.display.type === 1) return false;
        let productList     = $('#product_style_5_content_' + id + ' .list-product');
        let productBtnNext  = $('#product_style_5_header_' + id + ' .next');
        let productBtnPrev  = $('#product_style_5_header_' + id + ' .prev');
        let config = {
            infinite: true,
            dots:false,
            autoplay: true,
        ...
        $('#product_style_5_header_'+ id).attr('data-run', true);
    }

    function product_style_5_slider_load(categoryId, widgetId, options) {

        let productBox = $('#product_style_5_content_' + widgetId);
        ...
        let runSlick = $('#product_style_5_header_'+ widgetId).attr('data-run');
        ...
        if (typeof data_item[widgetId][categoryId] != 'undefined') {
            ...
            loading.hide();
            product_style_5_slider(widgetId, options);
            return false;
        }
        else {
            let data = {
                action  : 'widget_product_style_5::loadProduct',
                widgetId   : widgetId,
