<?php
class widget_slider_style_3 extends widget {
    function __construct() {
        parent::__construct('widget_slider_style_3', 'Slider 3', ['container' => true, 'position' => 'right']);
    ...
    function form( $left = [], $right = []) {
        ...
        $left[] = ['field' => 'item', 'type' => 'widget_slider_style_3::inputItem', 'arg' => ['number' => 0]];
        $right[] = ['field' => 'post_cate_id', 'label' =>'Danh mục bài viết', 'type' => 'cate_post_categories'];
        $right[] = ['field' => 'post_show', 'label' =>'Hiển thị', 'type' => 'number', 'value' => 2];
        parent::form($left, $right);
    }
    function widget() {
        $box = $this->container_box('widget_slider_style_3');
        echo $box['before']; ?>
    ...
    function css() {
        include_once 'assets/slider-style-3.css';
    }
    function default() {
        if(!isset($this->options->box)) $this->options->box = 'container';
        if(!isset($this->options->post_cate_id)) $this->options->post_cate_id = 0;
        if(!isset($this->options->post_show)) $this->options->post_show = 2;
...
Widget::add('widget_slider_style_3');